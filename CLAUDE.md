...
- **AU component descriptors** (3rd-party AUs) are stored as three contiguous 4-byte codes: `manufacturer + type + subtype`, all little-endian (i.e. reversed from how `auval` displays them). The type field is the anchor — we scan for `umua` / `xfua` / `fmua` / `imua` (reversed `aumu` / `aufx` / `aumf` / `aumi`) and read 4 bytes either side. `aumi` is `kAudioUnitType_MIDIProcessor` (separate from `aumf` `kAudioUnitType_MusicEffect`); both render as MIDI FX in the UI. Plug-ins whose manufacturer code contains non-printable bytes (e.g. Plugin Boutique's Scaler 2 with `00 00 69 01`) are accepted only if all four fields validate as printable ASCII — an existing gap in the Python whitelist that the Rust port papers over by being lenient about non-printable manufacturer codes.
- **Apple stock plug-ins** are NOT stored as standard 4CC triples. They live inside plug-in-slot records anchored on the `GAME` 4CC marker. Layout: `[?? <flag> 02] [12-byte null-padded ASCII display name] [GAME] [...]`. The byte at `-13` (immediately before the name field) is consistently `0x02`; the byte at `-14` is `0x02` for FX inserts and `0x00` for instrument slots. The full plug-in name lives in the 12-byte field — Compressor, Limiter, Bass Amp, ChromaVerb, Alchemy, etc. The actual `(type, subtype, manufacturer)` fingerprint is NOT recoverable from this format; the parser at `src-tauri/crates/lpx-parser/src/apple_stock.rs` synthesises a placeholder triple and surfaces the real name in `AURef.display_name`. Verified against `~/Music/Logic/ new idea.logicx` 2026-05-06 (lpx-explorer-0wo): 7 stock plug-ins recovered with zero false positives across all 35 `GAME` byte sequences.
- **Apple Drummer / Bass Player** uses a third storage shape: embedded JSON state strings keyed by region UUIDs. Each Drummer plug-in slot emits per-region snapshots of the form `{"selectedCharacterIdentifier":"<character>","selectedPersistentCharacterTypeIdentifier":"Type_<X>",...}`. The parser at `src-tauri/crates/lpx-parser/src/apple_drummer.rs` clusters JSON occurrences by byte-offset proximity (32 KB threshold), emits one `AURef` per cluster, and uses the *latest* `Type_X` identifier as the active character. Known characters: `Type_AcousticDrummerV2` → "Drummer", `Type_ElectricBassV2` → "Bass Player". Verified against `~/Music/Logic/ new idea.logicx` 2026-05-06 (lpx-explorer-0ph): 6 JSON snapshots (5 × Acoustic Drummer + 1 × Electric Bass) collapse to one cluster representing the Bass Player on track 2, lifting `find_aus` from 12/13 to 13/13 on this project.
...
- **NSKeyedArchive blobs** are spliced throughout `ProjectData`. They begin with `bplist00` and contain serialised Cocoa objects — Smart Controls layouts, plug-in parameter mappings, channel UUIDs (`_WsChannelUUID`), automation curve points (`MAGraphPoint`). They do **not** contain user-facing track names or AU descriptors (verified empirically 2026-05-06, lpx-explorer-3iv spike). Misleadingly, the `WsPluginIdentity` class inside these blobs refers to Smart Controls UI elements (Smart Knob, Smart Button), not audio plug-in identity. Useful eventually for extracting Smart Controls metadata; not on the path to track-name or plug-in-identity recovery.
...
**Track names without recorded regions** also live in the binary section as registry records (the `TRACK_SIGNATURE_KIND` table at `lpx_inspect.py:516`), not in NSKeyedArchive blobs (verified empirically against `~/Music/Logic/ new idea.logicx` 2026-05-06 — see lpx-explorer's `lpx-explorer-3iv` spike). When a renamed-but-unrecorded track shows as `"Audio N"` / `"Inst N"`, the most common cause is a missing 2-byte signature in the whitelist. Known additions beyond what the Python whitelist lists: `b"\xda\x11"` (instrument — Apple stock-instrument tracks like Bass) and `b"\xe7\x10"` (instrument — Alchemy / sampler tracks). Note the close collision between `b"\xe7\x10"` (instrument) and `b"\xe7\x11"` (folder, atmosphere/pad-cluster) — only the high byte distinguishes them.
